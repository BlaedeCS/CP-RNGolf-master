class Play extends Phaser.Scene {
    ...
    init() {
        ...
        this.SHOT_VELOCITY_Y_MINIMUM = 700
    ...
    create() {
        ...
        this.oneWayRight = true

        // add pointer input
        this.input.on('pointerdown', (pointer) => {
            let shotDirection = pointer.y <= this.ball.y ? 1 : -1
            let shotDirXtion = pointer.x <= this.ball.x ? -1 : 1
            this.ball.body.setVelocityX(Phaser.Math.Between(-this.SHOT_VELOCITY_X,0)*shotDirXtion)
            this.ball.body.setVelocityY(Phaser.Math.Between(this.SHOT_VELOCITY_Y_MINIMUM,this.SHOT_VELOCITY_Y_MAX) * shotDirection)
        })
    ...
    update() {
        if (this.oneWayRight) {
            if (this.oneWay.x >= width - this.oneWay.width / 2) {
                this.oneWayRight = false
                this.oneWay.setX(this.oneWay.x - 1) 
            } else {
                this.oneWay.setX(this.oneWay.x + 1) 
            }
            
        } else {
            if (this.oneWay.x <= this.oneWay.width / 2) {
                this.oneWayRight = true
                this.oneWay.setX(this.oneWay.x + 1) 
            } else {
                this.oneWay.setX(this.oneWay.x - 1) 
